        ...
        if hidden_channels is None or num_layers == 1:
            if gnn_type == 'GPS':
                ...
                for i in range(num_layers):
                    ...
                    layers.append(gnn(mlp, train_eps=True))
            else:
            ...
            if gnn_type == 'GPS':
                ...
                mlp1 = nn.Linear(in_channels, hidden_channels)
                layers.append(gnn(mlp1, train_eps=True))
                for i in range(num_layers-2):
                    mlp = nn.Linear(hidden_channels, hidden_channels)
                    layers.append(gnn(mlp, train_eps=True))
                mlp2 = nn.Linear(hidden_channels, out_channels)
                layers.append(gnn(mlp2, train_eps=True))
            else:
                raise ValueError(f"Unsupported GNN layer type: {gnn_type}")
        return layers
